package work.hn.pojo;
// default package



/**
 * AbstractHn895 entity provides the base persistence definition of the Hn895 entity. @author MyEclipse Persistence Tools
 */

public abstract class AbstractHn895  implements java.io.Serializable {


    // Fields    

     private Integer id;
     private String time;
     private String no;
     private String name;
     private Integer bmsb;
     private Double xlBmsb;
     private Integer yxcf;
     private Double xlYxcf;
     private Integer lrxg;
     private Double xlLrxg;
     private Integer lrsq;
     private Double xlLrsq;
     private Integer jhxg;
     private Double xlJhxg;
     private Integer jhsq;
     private Double xlJhsq;
     private Integer sbyy;
     private Double xlSbyy;
     private Integer cslr;
     private Double xlCslr;
     private Integer zyfs;
     private Double xlZyfs;
     private Integer shsh;
     private Double xlShsh;
     private Integer shsb;
     private Double xlShsb;
     private Integer rlcs;
     private Double xlRlcs;
     private Integer rlfs;
     private Double xlRlfs;
     private Integer rlsb;
     private Double xlRlsb;
     private Integer lrcc;
     private Integer jhcc;
     private Double lrccl;
     private Double jhccl;
     private Integer lrtp;
     private Double lrtpl;
     private Double ccl;
     private Integer cx;
     private Double cxl;
     private Double zhcl;
     private Integer ywl;
     private Double sumxl;
     private Integer zx;
     private Integer xz;


    // Constructors

    /** default constructor */
    public AbstractHn895() {
    }

    
    /** full constructor */
    public AbstractHn895(String time, String no, String name, Integer bmsb, Double xlBmsb, Integer yxcf, Double xlYxcf, Integer lrxg, Double xlLrxg, Integer lrsq, Double xlLrsq, Integer jhxg, Double xlJhxg, Integer jhsq, Double xlJhsq, Integer sbyy, Double xlSbyy, Integer cslr, Double xlCslr, Integer zyfs, Double xlZyfs, Integer shsh, Double xlShsh, Integer shsb, Double xlShsb, Integer rlcs, Double xlRlcs, Integer rlfs, Double xlRlfs, Integer rlsb, Double xlRlsb, Integer lrcc, Integer jhcc, Double lrccl, Double jhccl, Integer lrtp, Double lrtpl, Double ccl, Integer cx, Double cxl, Double zhcl, Integer ywl, Double sumxl, Integer zx, Integer xz) {
        this.time = time;
        this.no = no;
        this.name = name;
        this.bmsb = bmsb;
        this.xlBmsb = xlBmsb;
        this.yxcf = yxcf;
        this.xlYxcf = xlYxcf;
        this.lrxg = lrxg;
        this.xlLrxg = xlLrxg;
        this.lrsq = lrsq;
        this.xlLrsq = xlLrsq;
        this.jhxg = jhxg;
        this.xlJhxg = xlJhxg;
        this.jhsq = jhsq;
        this.xlJhsq = xlJhsq;
        this.sbyy = sbyy;
        this.xlSbyy = xlSbyy;
        this.cslr = cslr;
        this.xlCslr = xlCslr;
        this.zyfs = zyfs;
        this.xlZyfs = xlZyfs;
        this.shsh = shsh;
        this.xlShsh = xlShsh;
        this.shsb = shsb;
        this.xlShsb = xlShsb;
        this.rlcs = rlcs;
        this.xlRlcs = xlRlcs;
        this.rlfs = rlfs;
        this.xlRlfs = xlRlfs;
        this.rlsb = rlsb;
        this.xlRlsb = xlRlsb;
        this.lrcc = lrcc;
        this.jhcc = jhcc;
        this.lrccl = lrccl;
        this.jhccl = jhccl;
        this.lrtp = lrtp;
        this.lrtpl = lrtpl;
        this.ccl = ccl;
        this.cx = cx;
        this.cxl = cxl;
        this.zhcl = zhcl;
        this.ywl = ywl;
        this.sumxl = sumxl;
        this.zx = zx;
        this.xz = xz;
    }

   
    // Property accessors

    public Integer getId() {
        return this.id;
    }
    
    public void setId(Integer id) {
        this.id = id;
    }

    public String getTime() {
        return this.time;
    }
    
    public void setTime(String time) {
        this.time = time;
    }

    public String getNo() {
        return this.no;
    }
    
    public void setNo(String no) {
        this.no = no;
    }

    public String getName() {
        return this.name;
    }
    
    public void setName(String name) {
        this.name = name;
    }

    public Integer getBmsb() {
        return this.bmsb;
    }
    
    public void setBmsb(Integer bmsb) {
        this.bmsb = bmsb;
    }

    public Double getXlBmsb() {
        return this.xlBmsb;
    }
    
    public void setXlBmsb(Double xlBmsb) {
        this.xlBmsb = xlBmsb;
    }

    public Integer getYxcf() {
        return this.yxcf;
    }
    
    public void setYxcf(Integer yxcf) {
        this.yxcf = yxcf;
    }

    public Double getXlYxcf() {
        return this.xlYxcf;
    }
    
    public void setXlYxcf(Double xlYxcf) {
        this.xlYxcf = xlYxcf;
    }

    public Integer getLrxg() {
        return this.lrxg;
    }
    
    public void setLrxg(Integer lrxg) {
        this.lrxg = lrxg;
    }

    public Double getXlLrxg() {
        return this.xlLrxg;
    }
    
    public void setXlLrxg(Double xlLrxg) {
        this.xlLrxg = xlLrxg;
    }

    public Integer getLrsq() {
        return this.lrsq;
    }
    
    public void setLrsq(Integer lrsq) {
        this.lrsq = lrsq;
    }

    public Double getXlLrsq() {
        return this.xlLrsq;
    }
    
    public void setXlLrsq(Double xlLrsq) {
        this.xlLrsq = xlLrsq;
    }

    public Integer getJhxg() {
        return this.jhxg;
    }
    
    public void setJhxg(Integer jhxg) {
        this.jhxg = jhxg;
    }

    public Double getXlJhxg() {
        return this.xlJhxg;
    }
    
    public void setXlJhxg(Double xlJhxg) {
        this.xlJhxg = xlJhxg;
    }

    public Integer getJhsq() {
        return this.jhsq;
    }
    
    public void setJhsq(Integer jhsq) {
        this.jhsq = jhsq;
    }

    public Double getXlJhsq() {
        return this.xlJhsq;
    }
    
    public void setXlJhsq(Double xlJhsq) {
        this.xlJhsq = xlJhsq;
    }

    public Integer getSbyy() {
        return this.sbyy;
    }
    
    public void setSbyy(Integer sbyy) {
        this.sbyy = sbyy;
    }

    public Double getXlSbyy() {
        return this.xlSbyy;
    }
    
    public void setXlSbyy(Double xlSbyy) {
        this.xlSbyy = xlSbyy;
    }

    public Integer getCslr() {
        return this.cslr;
    }
    
    public void setCslr(Integer cslr) {
        this.cslr = cslr;
    }

    public Double getXlCslr() {
        return this.xlCslr;
    }
    
    public void setXlCslr(Double xlCslr) {
        this.xlCslr = xlCslr;
    }

    public Integer getZyfs() {
        return this.zyfs;
    }
    
    public void setZyfs(Integer zyfs) {
        this.zyfs = zyfs;
    }

    public Double getXlZyfs() {
        return this.xlZyfs;
    }
    
    public void setXlZyfs(Double xlZyfs) {
        this.xlZyfs = xlZyfs;
    }

    public Integer getShsh() {
        return this.shsh;
    }
    
    public void setShsh(Integer shsh) {
        this.shsh = shsh;
    }

    public Double getXlShsh() {
        return this.xlShsh;
    }
    
    public void setXlShsh(Double xlShsh) {
        this.xlShsh = xlShsh;
    }

    public Integer getShsb() {
        return this.shsb;
    }
    
    public void setShsb(Integer shsb) {
        this.shsb = shsb;
    }

    public Double getXlShsb() {
        return this.xlShsb;
    }
    
    public void setXlShsb(Double xlShsb) {
        this.xlShsb = xlShsb;
    }

    public Integer getRlcs() {
        return this.rlcs;
    }
    
    public void setRlcs(Integer rlcs) {
        this.rlcs = rlcs;
    }

    public Double getXlRlcs() {
        return this.xlRlcs;
    }
    
    public void setXlRlcs(Double xlRlcs) {
        this.xlRlcs = xlRlcs;
    }

    public Integer getRlfs() {
        return this.rlfs;
    }
    
    public void setRlfs(Integer rlfs) {
        this.rlfs = rlfs;
    }

    public Double getXlRlfs() {
        return this.xlRlfs;
    }
    
    public void setXlRlfs(Double xlRlfs) {
        this.xlRlfs = xlRlfs;
    }

    public Integer getRlsb() {
        return this.rlsb;
    }
    
    public void setRlsb(Integer rlsb) {
        this.rlsb = rlsb;
    }

    public Double getXlRlsb() {
        return this.xlRlsb;
    }
    
    public void setXlRlsb(Double xlRlsb) {
        this.xlRlsb = xlRlsb;
    }

    public Integer getLrcc() {
        return this.lrcc;
    }
    
    public void setLrcc(Integer lrcc) {
        this.lrcc = lrcc;
    }

    public Integer getJhcc() {
        return this.jhcc;
    }
    
    public void setJhcc(Integer jhcc) {
        this.jhcc = jhcc;
    }

    public Double getLrccl() {
        return this.lrccl;
    }
    
    public void setLrccl(Double lrccl) {
        this.lrccl = lrccl;
    }

    public Double getJhccl() {
        return this.jhccl;
    }
    
    public void setJhccl(Double jhccl) {
        this.jhccl = jhccl;
    }

    public Integer getLrtp() {
        return this.lrtp;
    }
    
    public void setLrtp(Integer lrtp) {
        this.lrtp = lrtp;
    }

    public Double getLrtpl() {
        return this.lrtpl;
    }
    
    public void setLrtpl(Double lrtpl) {
        this.lrtpl = lrtpl;
    }

    public Double getCcl() {
        return this.ccl;
    }
    
    public void setCcl(Double ccl) {
        this.ccl = ccl;
    }

    public Integer getCx() {
        return this.cx;
    }
    
    public void setCx(Integer cx) {
        this.cx = cx;
    }

    public Double getCxl() {
        return this.cxl;
    }
    
    public void setCxl(Double cxl) {
        this.cxl = cxl;
    }

    public Double getZhcl() {
        return this.zhcl;
    }
    
    public void setZhcl(Double zhcl) {
        this.zhcl = zhcl;
    }

    public Integer getYwl() {
        return this.ywl;
    }
    
    public void setYwl(Integer ywl) {
        this.ywl = ywl;
    }

    public Double getSumxl() {
        return this.sumxl;
    }
    
    public void setSumxl(Double sumxl) {
        this.sumxl = sumxl;
    }

    public Integer getZx() {
        return this.zx;
    }
    
    public void setZx(Integer zx) {
        this.zx = zx;
    }

    public Integer getXz() {
        return this.xz;
    }
    
    public void setXz(Integer xz) {
        this.xz = xz;
    }
   








}