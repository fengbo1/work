...
<script type="text/javascript">
 ...
 function change(cp){
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			if(arr[1]=='plate')
			{
				document.getElementById("poold").innerHTML=arr[4];
				document.getElementById("partd").innerHTML=arr[5];
				document.getElementById("aread").innerHTML=arr[6];
				document.getElementById("factord").innerHTML=arr[7];
			}
			if(arr[1]=='pool')
			{
	...
	//alert(part); 
	xmlhttp.open("GET","searchselect.action?currentPage="+cp+"&word="+word+"&plate="+plate+"&key="+key+"&pool="+pool+"&part="+part+"&area="+area+"&factor="+factor,true);
	xmlhttp.send();
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
			...
			<table align="center" style="width: 1180px;height:500px">
			<tr>
			<td>
				...
				<tr height="50px" class="qq">
					...
					<div style="width: 150px; float: left">
							...
							<select id="plate" name="plate"
								style="width: 100px" onchange="setkey('plate');change(1)">
								<option value="0">
									请选择
								</option>
									<c:forEach items="${list}" var="plt" varStatus="status">
 										<option value="${plt}">${plt}</option>
 									</c:forEach>
							</select> 
						</div>
								
						<div id="poold" style="width: 150px; float: left">
							任务池
							<select id="pool" name="pool" style="width: 20px"
								onchange="change('pool')">
							</select>
						</div>
						...
						<!-- <input style="width: 50px;" type="submit" value="搜索"/> -->
					</td>
				</tr>
				<tr>
					<td colspan="8">
