...
<script type="text/javascript">
 ...
 function change(cp){
	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			if(arr[1]=='pool')
			{
				document.getElementById("partd").innerHTML=arr[3];
				document.getElementById("factord").innerHTML=arr[4];
			}
			else if(arr[1]=='part')
			{
				document.getElementById("factord").innerHTML=arr[3];
			}
	...
	//alert(part); 
	xmlhttp.open("GET","caseselect.action?currentPage="+cp+"&word="+word+"&plate="+plate+"&key="+key+"&pool="+pool+"&part="+part+"&factor="+factor+"&mg="+mg,true);
	xmlhttp.send();
 ...
 function view(idd)
	...
	{
		var up = "to_update";
		window.open("<%=path%>/casejumptodetail.action?id="+idd+"&type="+up,"","height=700,width=950,top=100,left=290,scrollbars=yes,resizable=yes");
		window.close();
	}
	function delrule(idd)
	{
		var up = "to_update";
		window.open("<%=path%>/delcase.action?id="+idd,"","height=10,width=10,top=400,left=600");
		window.close();
	}
	function xinzeng()
	{
		var plate=encodeURI('外汇业务');
		window.open("<%=path%>/casetoadd.action?plate="+plate,"","height=700,width=950,top=100,left=290,scrollbars=yes,resizable=yes");
	}
	function highlight()
	 {
 ...
 <link href="<%=path%>/css/table_back.css" rel="stylesheet" type="text/css">
  ...
  <body>
			<table align="center" style="width: 1100px;height:500px">
			<tr>
			<td>
				...
				<tr height="50px" class="qq">
					<td colspan="8">
						...
						<!-- 业务板块
								...
								style="width: 100px" onchange="change(plate)">
							</select> -->
						<div id="partd" style="width: 160px; float: left">
							环节
							<select id="part" name="part" style="width: 20px"
								onchange="change('part')">
							</select>
						</div>
						<div id="factord" style="width: 160px; float: left">
							要素
			...
			<div id="scroll" align="center" style="overflow-y: scroll; overflow-x: hidden;height:430px">
